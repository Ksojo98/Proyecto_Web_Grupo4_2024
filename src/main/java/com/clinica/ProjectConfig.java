...
public class ProjectConfig implements WebMvcConfigurer {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .requestMatchers("/login", "/registro", "/activar-cuenta", "/recuperar", "/cambiar-contrasena").permitAll()
                .requestMatchers("/horarios/admin").hasRole("ADMIN")
    
                .anyRequest().authenticated()
                )
                .formLogin(form -> form
                .loginPage("/login")
                .defaultSuccessUrl("/", true) // Página a redirigir después de iniciar sesión
                .permitAll()
                )
